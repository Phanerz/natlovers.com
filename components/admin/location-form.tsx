...
// land in the form is the confirmation this needs  -  nothing gets set
// until the admin looks at what Nominatim found and accepts it.
//
// No icon picker: every marker on the public map is the same house
// pictogram now, sized by type (main_studio vs stockist) rather than
// admin-chosen, so there's nothing to pick here.
      ...
      <div className="space-y-3 rounded-lg border border-[#d4c5ab] bg-[#fffdf9] p-4">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div>
            ...
            <p className="text-xs text-forest-500">{hasCoordinates ? `${form.latitude}, ${form.longitude}` : "Not yet located."}</p>
          </div>
          <button
            type="button"
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        <label className="block space-y-2 text-sm text-forest-700">
          ...
          <input
            ...
            placeholder='e.g. "Open by appointment" or "Mon-Sat, 09:00-17:00 WIB"'
            className={fieldClass}
          />
        </label>

        <label className="block space-y-2 text-sm text-forest-700">
          <span className="muted">Contact (optional)</span>
          <input
            value={form.contact}
            onChange={(event) => onChange({...form, contact: event.target.value})}
            placeholder='e.g. "+62 812-2697-007"'
            className={fieldClass}
          />
        </label>
      </div>
