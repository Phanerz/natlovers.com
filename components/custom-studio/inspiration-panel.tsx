...
import {AlertCircle, Info, Loader2, Plus, RotateCcw, UploadCloud, X} from "lucide-react";
...
// The right column: inspiration photographs and a free-text note. Limits
// shown here come from lib/upload-limits.ts, which is the same constant the
// upload route enforces — the copy under the dropzone cannot drift away
...
export function InspirationPanel({
  ...
  async function upload(files: File[]) {
    ...
    // rather than after a slow upload that the server would reject anyway.
    ...
    <div className="space-y-5">
      <div className="space-y-2.5">
        <div className="flex items-center gap-1.5">
          <p className="text-[10px] font-semibold uppercase tracking-[0.22em] text-forest-500">Your inspiration</p>
          <span title="Photos help the studio understand what you're picturing.">
            <Info className="h-3 w-3 text-forest-400" />
          </span>
        </div>
        ...
        <div
          ...
          className={`flex cursor-pointer flex-col items-center justify-center gap-1.5 rounded-2xl border-2 border-dashed px-4 py-8 text-center transition-colors duration-150 ${
        ...
        >
          {uploading ? (
            ...
            <UploadCloud className="h-6 w-6 text-forest-500" />
          )}
          <p className="text-[12px] font-medium text-forest-800">
            {uploading ? "Uploading..." : "Drop photos here"}
          </p>
          {!uploading ? <p className="text-[11px] text-forest-500">or click to browse</p> : null}
        ...
        {rejected ? (
          <p className="flex items-start gap-1.5 text-[11px] leading-relaxed text-[#a4553c]">
        ...
        {failed ? (
          <div className="flex items-start justify-between gap-2 rounded-xl border border-[#e6c4b6] bg-[#fdf1ec] px-3 py-2">
            <p className="flex items-start gap-1.5 text-[11px] leading-relaxed text-[#a4553c]">
              <AlertCircle className="mt-0.5 h-3 w-3 shrink-0" />
              {failed.message}
            </p>
            {failed.files.length ? (
              <button
                ...
                className="flex shrink-0 items-center gap-1 rounded-full border border-[#d9a992] px-2.5 py-1 text-[10px] font-medium text-[#a4553c]"
              >
        ...
        {images.length ? (
          <div className="grid grid-cols-4 gap-2">
            {images.map((image) => (
              ...
              >
                <Image src={image.url} alt="Inspiration" fill sizes="80px" className="object-cover" />
                <button
                  type="button"
                  onClick={() => void remove(image)}
                  aria-label="Remove photo"
                  className="absolute right-1 top-1 flex h-5 w-5 items-center justify-center rounded-full bg-black/55 text-white opacity-0 transition-opacity group-hover:opacity-100 focus:opacity-100"
                >
                  <X className="h-3 w-3" />
                </button>
              </div>
        ...
        {!signedIn ? (
          <p className="text-[11px] leading-relaxed text-forest-500">
      ...
      <div className="space-y-2">
        <div className="flex items-center justify-between">
          <p className="text-[10px] font-semibold uppercase tracking-[0.22em] text-forest-500">Anything else?</p>
        ...
        <textarea
          ...
          className="w-full resize-none rounded-2xl border border-[#ddd5c4] bg-white px-3.5 py-3 text-sm leading-relaxed text-forest-900 outline-none placeholder:text-forest-300 focus:border-forest-600"
        />
